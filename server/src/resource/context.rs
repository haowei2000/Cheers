...
pub async fn handle(db: &PgPool, principal: &Principal, params: &Value) -> ResourceResult {
    ...
    if include.is_empty() || include.contains(&"info") {
        if let Ok(info) = channel_info::handle(db, principal, &chan_params).await {
            result["info"] = info;
        }
    }

    if include.is_empty() || include.contains(&"members_summary") {
        if let Ok(m) = members::handle(db, principal, &chan_params).await {
    ...
    if include.is_empty() || include.contains(&"recent_messages") {
        ...
        if let Ok(msgs) = messages::handle_read(db, principal, &msg_params).await {
