...
> **语言**：中文 | [English](AgentNexus门户与知识平台设计.md)
...
| 类型 | 说明 | 示例 |
|------|------|------|
| **内置 Bot（Built-in）** | 系统自带，不可删除 | Coordinator |
...
| 属性 | 说明 |
|------|------|
| **职责** | 提供使用引导、澄清表单、业务问答与路由建议 |
| **触发** | 可被 `@Coordinator` 显式触发；频道开启 `auto_assist` 时也可回答未 @ 的问题 |
| **知识源** | 系统使用说明、频道上下文、公共平台访问申请 API 规范等文档 |
| **当前标识** | `username=Coordinator`，`bot_id=bot-helper-001` |

### 2.4 历史名称说明

早期设计把“帮助 Bot”和“调度 Bot”拆成 Helper / Orchestrator。当前公开版本已收敛为统一的 Coordinator。历史术语只作为设计背景保留，用户文档和新功能说明应统一使用 Coordinator。
...
| 场景 | 处理方式 | 负责方 |
|------|----------|--------|
| **仅需问答** | Coordinator 优先回答 | AgentNexus 中心 |
| **需引用部门能力** | 在回答中建议 @部门bot | Coordinator 建议，用户或自动接手 |
...
| 序号 | 交付物 | 实现状态 | 实现说明 |
|------|--------|----------|----------|
| 1 | Coordinator 作为系统内置 Bot | ✅ | `bot_id=bot-helper-001`，`username=Coordinator`，可加入频道 |
| 2 | Coordinator 触发模式配置 | ✅ | 频道级 `auto_assist` 控制未 @ 场景是否自动回复；显式 `@Coordinator` 始终按频道成员关系触发 |
| 3 | Coordinator 职责明确 | ✅ | 集使用帮助、澄清、业务问答、路由建议能力于一体 |
| 4 | 自动接手机制 | ✅ | Coordinator 回复含「建议 @xxx」时，可自动触发被建议 Bot |
| 5 | 自动接手 UI | ✅ | WebSocket 推送 bot_processing 事件，前端显示「@xxx 正在处理...」 |
| 6 | Bot 层级与 @ 规则 | ✅ | 内置 Bot 可建议 @ 部门 Bot；外部 Bot 不可 @ 另一 Bot |
| 7 | 所有 Bot 支持澄清 | 部分 | Coordinator 已支持；HTTP Bot 仍按 PromptTemplate 能力逐步扩展 |
| 8 | 资源监控（基础版） | 规划中 | 后续提供按 Bot 汇总任务数、平均耗时、成功率的可视化看板 |
...
| 5 | Coordinator 知识扩展 | 纳入申请 API 文档，可回答申请流程 |
...
| 2 | Coordinator 学习 Bot 能力 | 根据能力清单给出更精准的 @ 建议 |
| 3 | 资源监控增强 | 更细粒度监控与告警 |
...
- 频道表使用 `auto_assist` 控制未 @ 场景是否自动触发 Coordinator。
- Coordinator 使用本地 LLM 时沿用 `HELPER_LLM_*` 环境变量作为兼容配置。
- Bot 运行与路由仍走 Bot Runtime 管线，不再暴露早期设计里的独立 Orchestrator settings API。
...
| POST /api/v1/channels | 创建频道，可设置 `auto_assist` |
| POST /api/v1/channels/{channel_id}/members | 将 Coordinator 或外部 Bot 加入频道 |
| GET /api/v1/channels/{channel_id}/context | 查看频道上下文 |
| GET /docs/agent-bridge/discovery | Agent Bridge 机器可读发现入口 |
...
- [系统管理说明书](../help/系统管理说明书.md)（Coordinator 配置、Agent Bridge 接入、排查）
...
- 内置体系已收敛为统一 `Coordinator`（`bot-helper-001`），承担帮助、协作、路由建议能力。
- `auto_assist` 支持频道级自动接管；当前公开文档以频道配置为准。
- 自动接手机制与前端“正在处理”提示已实现，可配置开关。
- SSE 消息流式接口已落地，支持更细粒度的处理反馈。
- 资源监控看板仍作为后续规划，不在公开帮助文档中列为可用接口。
