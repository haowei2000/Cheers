> **语言**：中文 | [English](AgentNexus_概要设计说明书_v2.0.md)
...
| 接口路径 | 方法 | 描述 |
|----------|------|------|
| /api/v1/channels | GET / POST | 获取/创建频道列表 |
| /api/v1/channels/\{id\}/messages | GET / POST | 获取消息历史 / 发送消息 |
| /api/v1/channels/\{id\}/members | GET / POST / DELETE | 查询/添加/移除频道成员（含 Bot） |
| /api/v1/channels/\{id\}/context | GET / PUT | 读取/更新频道四层 Context Store |
...
| /api/v1/files/upload | POST | 上传文件，触发转换管道 |
| /api/v1/files/\{file\_id\}/status | GET | 查询文件转换状态 |
...
- 内置 Bot 体系已收敛为统一 `Coordinator`（`bot-helper-001`），用户主入口统一为 `@Coordinator`。
- Bot 执行默认主链路为 `LLMBotAdapter`（模型 + 模板）；OpenClaw HTTP/WS Adapter 保留为可选接入能力。
- 频道模型新增 `auto_assist`，支持频道级自动接管与协作触发。
- 消息链路新增 SSE 流式接口（`/api/v1/channels/{channel_id}/messages/stream`）。
- 文件上传主路径升级为预签名上传（`/api/v1/files/presign`），legacy 上传接口继续兼容。
...
| v2\.1 | 2026\-03\-16 | 根据代码实现完成对齐修订：内置 Bot 统一为 Coordinator；主链路默认 LLM Bot；补充 SSE、对象存储、workspace membership、image\_gen、MCP 导入与频道级 auto\_assist；明确公共平台与 A2A 仍属后续阶段。 | 维护更新 |
