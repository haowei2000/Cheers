...
> **语言**：中文 | [English](文件上传与SSE推理联调说明.md)
...
3. 前端拿到稳定的 `file_id`
4. 前端调用 `POST /api/v1/channels/{channel_id}/messages/stream`
...
```bash
curl -N -X POST http://localhost:8000/api/v1/channels/your-channel-id/messages/stream \
  -H "Content-Type: application/json" \
  -d '{
    "content": "@Coordinator 请结合文件回答",
...
- 上传时先请求 `/api/v1/files/presign`
- 使用返回的 `upload_url` 直接上传 RustFS
- 发送带附件消息时走 `/api/v1/channels/{channel_id}/messages/stream`
- 流式结果通过 SSE 实时更新当前消息气泡

非附件消息仍保留原有 `POST /api/v1/channels/{channel_id}/messages` 行为。
