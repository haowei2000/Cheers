...
把 OpenClaw agent 接入 AgentNexus，让它作为一个 "Agent Bridge Bot" 出现在频道里，被 `@mention` 时自动响应。
...
│                             │         │                              │
│  ┌──────────────────────┐   │         │  ┌────────────────────────┐  │
│  │ agentnexus plugin    │   │  WS     │  │ /ws/agent-bridge/control   │  │
│  │  (channel adapter)   │◄────────────│►│  （hello/join/left）   │  │
│  │                      │   │         │  └────────────────────────┘  │
│  │  BotSession          │   │  WS     │  ┌────────────────────────┐  │
│  │  ├── control client  │◄────────────│►│  /ws/agent-bridge/data     │  │
...
一个 OpenClaw `account` 对应一个 AgentNexus `Agent Bridge Bot`（`binding_type = "websocket"`）。
...
| 组件 | 在谁那 | 做什么 |
|---|---|---|
| `/ws/agent-bridge/control` | AgentNexus backend | 发 `hello`（membership 快照）+ 推 `channel_joined / channel_left` |
| `/ws/agent-bridge/data` | AgentNexus backend | 推 `message` 事件；接受 `reply / send` 帧；支持 `resume` |
...
- WS：`ws://localhost:8002/ws/agent-bridge/{control,data}`
...
弹窗显示**一次性** `bot_token`（`agb_...`），**立刻复制**。关闭后只能 `rotate`。
...
```bash
AGENTNEXUS_BOT_TOKEN=agb_粘贴刚复制的 \
AGENTNEXUS_CONTROL_URL=ws://localhost:8002/ws/agent-bridge/control \
AGENTNEXUS_DATA_URL=ws://localhost:8002/ws/agent-bridge/data \
  npm run demo
```
  ...
  "channels": {
    "agentnexus": {
      ...
      "accounts": {
        "my-bot": {
          "enabled": true,
          "botToken": "agb_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx",
...
```bash
# 需要一个 bridge debug token；.env 里的 AGENT_BRIDGE_TOKEN
curl -H "X-Agent-Bridge-Token: <BRIDGE_TOKEN>" \
  http://localhost:8002/api/v1/agent-bridge/status
```
...
"channels.agentnexus": {
  ...
  "accounts": {
    "<accountId>": {
      "enabled": true,                 // 账号级开关
      "botToken": "agb_...",           // 必填；AgentNexus 创建 Agent Bridge Bot 时一次性返回
      "controlUrl": "ws://host/ws/agent-bridge/control",  // 必填
      "dataUrl":    "ws://host/ws/agent-bridge/data",     // 必填
      "advanced": {
...
```
Authorization: Bearer agb_...
```

Header 是首选；调试时可用 `?token=agb_...` 查询参数，但生产不推荐（会进 access log）。
...
多 account 场景下，每个 OpenClaw account 建议对应一个 AgentNexus Agent Bridge Bot 和一份独立 bot token。若希望后台数据里直接显示 OpenClaw 配置里的 accountId，可在 Bot 的 `binding_config` 中写入：
...
- 每个 Agent Bridge Bot 有自己独立的 `bot_token`，在 AgentNexus 创建 Bot 时一次性返回（`agb_<43chars>`）
...
- **事件日志保留策略** —— `agent_bridge_events` 当前无限增长；需要 cron prune
...
`agent_bridge_events` 表随着 dispatch 数量单调增长。生产前需要：

```sql
-- 例如只保留 7 天
DELETE FROM agent_bridge_events WHERE created_at < now() - interval '7 days';
```
...
- AgentNexus 后端 bridge 路由：[`backend/app/api/v1/agent-bridge_bridge/routes.py`](../../backend/app/api/v1/agent-bridge_bridge/routes.py)
- AgentNexus adapter：[`backend/app/services/adapters/websocket_bot.py`](../../backend/app/services/adapters/websocket_bot.py)
- 会话/派发注册表：[`backend/app/services/agent_bridge/registry.py`](../../backend/app/services/agent_bridge/registry.py)
- 事件日志 / resume：[`backend/app/services/agent_bridge/event_log.py`](../../backend/app/services/agent_bridge/event_log.py)
