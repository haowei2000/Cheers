...
- **当前阶段**：里程碑 3（智能调度）已实现 Coordinator 与质量监控；门户阶段一待实施
- **当前 todo**：M1～M3 按规划完成；门户阶段一按《AgentNexus门户与知识平台设计》§七实施；验收见 [功能测试清单](功能测试清单.md)
...
- [x] **M1-01** 项目骨架：目录结构、依赖（FastAPI/React/Docker/SQLite/Redis）、.env 与 .gitignore、README
- [x] **M1-02** 数据层：SQLite 主库表结构（Workspace/Channel/User/BotAccount/ChannelMembership/Message 等），迁移方式（Alembic）
...
- [x] **M1-11** Docker Compose：后端 + 前端 + Redis（可选）+ 卷（SQLite 持久化）；一键启停可复现验收场景
...
- [x] **P1-01** Orchestrator 作为系统内置 Bot：与引导 Bot 并列，可加入频道
- [x] **P1-02** Orchestrator 触发模式配置：必须 @ 才回答 / 直接回答未 @ 的问题（可关闭）
- [x] **P1-03** 引导 Bot 职责明确：仅回答系统使用问题，必须 @ 才活跃
- [x] **P1-04** Bot 层级与 @ 规则：内置 Bot 可 @ Bot/人；外部 Bot 仅可 @ 人
- [x] **P1-05** 自动接手机制：Orchestrator 建议后，被建议 Bot 可自动接手（可关闭）
- [x] **P1-06** 自动接手 UI：显示部门 Bot 回复框 +「正在处理...」
- [ ] **P1-07** 所有 Bot 支持澄清：信息不足时可向用户提问（引导 Bot 已支持；HTTP Bot 待扩展）
- [x] **P1-08** 资源监控基础版：Bot 任务数、平均耗时（/api/tasks/stats）
