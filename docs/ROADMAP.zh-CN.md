...
> 面向工程的**执行**路线图（逐里程碑的落地顺序、验收门、改造项 R1–R14）见 [docs/arch/ROADMAP.md](arch/ROADMAP.md)——那是独立的工程执行文档；本文是面向公众的产品路线图。
...
> **不做"更好的 Slack"，做"Agent 舰队的治理工作面"。聊天是界面，治理才是产品。**

团队正在走向同时运行 5–20 个 agent 的时代。到那时，真正难的问题——谁能指挥哪个 agent、
它花谁的预算、谁批准了哪个动作、agent 们在哪协作——只有**拥有身份、权限与审计模型**的
一方能回答。这正是 Cheers 在建的东西：bot 是 Cheers **自有**治理模型中的一等成员，
而不是寄居在别人聊天应用里的客人。（完整竞品分析见 [COMPARISON.zh-CN.md](COMPARISON.zh-CN.md)。）

三步走：

1. **把治理做到无可争辩**——审批体验做到像 review 一个 PR 一样顺滑（diff 预览、一键
   批/拒、决定记入审计）；per-bot / per-channel 的**费用预算**，超限自动暂停并升级审批
   （是治理，不只是统计）；**审计导出**与保留策略，把 Viewboard 留痕变成自部署企业的
   合规能力。
2. **官方 Slack/Discord 桥作为获客漏斗**——在 Slack 里 `@` 一个 Cheers bot；工作在
   Cheers 频道内、受完整权限与审计约束地执行；结果回帖并附完整留痕链接。团队无需搬家
   即可开始用；桥是漏斗，平台是产品。
3. **多 Agent 编排**——频道即受治理的"会议室"：多个外部 agent（ACP/MCP）与人类在同一
   授权矩阵下共事，Workbench 是共享黑板，人类是审批节点。当 agent 本身商品化，稀缺的
   是它们共享的、受治理的空间。
