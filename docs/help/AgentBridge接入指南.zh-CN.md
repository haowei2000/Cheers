# Cheers Agent Bridge 接入指南

> **语言**：中文 | [English](AgentBridge接入指南.md)

本文档说明外部 Agent 如何接入 Cheers：
...
推荐 ACP 路径和遗留 OpenClaw 路径都使用 Cheers 的 **Agent Bridge Bot**。用户在频道或 DM 中 `@Bot` 后，Cheers 通过两条 WebSocket 把任务推给外部 provider，provider 再把流式文本、最终回复和附件回传给 Cheers。
...
Agent Bridge Bot 是 Cheers 中专门给外部 provider 使用的 Bot 账号。创建后会得到一次性明文 `botToken`，格式类似 `agb_...`。
...
- ACP Connector 是推荐的 Agent Bridge provider，不需要改 Cheers 后端协议。OpenClaw provider 仅保留为废弃/遗留入口。
...
## 二、在 Cheers 创建 Agent Bridge Bot
...
响应里的 `data.bot.bot_token` 是外部 provider 要保存的 `botToken`。如果丢失，只能在 Cheers 里 rotate token。

---

## 三、连接 OpenClaw（废弃/遗留）

> OpenClaw channel package 已停用，仓库不再保留 `packages/openclaw-channel-cheers` 源码包。已有部署请迁移到 `/acp-bridge` 和本地运行的 ACP Agent；新接入不要再使用 OpenClaw 包路径。
...
也可以用脚本快速写入配置。先确认插件已安装并在 `openclaw plugins list | grep cheers` 中显示 `loaded`，再执行：

```bash
CHEERS_BOT_TOKEN='agb_替换成最新token' python3 - <<'PY'
import json
import os
import sys
from pathlib import Path

token = os.environ.get("CHEERS_BOT_TOKEN", "").strip()
if not token.startswith("agb_"):
    sys.exit("CHEERS_BOT_TOKEN 必须是最新的 agb_... token")

plugin_dir = Path.home() / ".openclaw" / "extensions" / "cheers"
if not plugin_dir.exists():
    sys.exit(f"插件目录不存在：{plugin_dir}\n请先安装插件，再添加 channels.cheers")

p = Path.home() / ".openclaw" / "openclaw.json"
backup = p.with_suffix(".json.bak.add-cheers")
...
entries["cheers"] = {"enabled": True}

allow = plugins.setdefault("allow", [])
if "cheers" not in allow:
    allow.append("cheers")

data.setdefault("channels", {})["cheers"] = {
    "enabled": True,
    "accounts": {
        "remote-bot": {
            "enabled": True,
            "botToken": token,
            "controlUrl": "wss://cheers.example.com/ws/agent-bridge/control",
            "dataUrl": "wss://cheers.example.com/ws/agent-bridge/data",
            "advanced": {
                "reconnectBaseMs": 1000,
                "reconnectMaxMs": 30000,
                "heartbeatIntervalMs": 30000,
...
如果 OpenClaw 报 `channels.cheers: unknown channel id: cheers`，通常是还没安装或启用插件就先写入了 channel 配置。先删除 `channels.cheers`，安装并确认插件 `loaded` 后再执行上面的脚本。

字段说明：

| 字段 | 必填 | 说明 |
|------|------|------|
| `botToken` | 是 | Cheers 创建 Bot 时返回的 `agb_...` |
| `controlUrl` | 是 | Cheers control WS |
| `dataUrl` | 是 | Cheers data WS |
| `enabled` | 否 | 临时启停该 account，默认 `true` |
| `advanced.sendAckTimeoutMs` | 否 | 大文件或长任务建议设置到 10 分钟左右 |

一个 OpenClaw 进程可以挂多个 account。每个 account 对应一个 Cheers Agent Bridge Bot。
...
openclaw daemon restart
openclaw channels status --probe
openclaw channels logs | grep cheers | tail -50
```
...
```text
cheers: openclaw-main ready bot_id=...
cheers: openclaw-main inbound channel=... task=...
```
...
```text
@openclaw-main 你好，确认一下你已经连上 Cheers
```

如果 Bot 在频道中、token 正确、OpenClaw 插件在线，Cheers 会显示 Bot 的流式回复或最终回复。

---

## 四、连接 ACP 协议 Agent：以 OpenCode ACP 为例

ACP Connector 适合把本机 stdio ACP agent 接到 Cheers。它本身负责：

- 连接 Cheers 的 control/data WS；
- 启动本机 ACP agent，例如 `opencode acp`；
- 把 Cheers 用户消息转成 ACP session prompt；
- 把 ACP 的流式输出、最终回复和文件资源回传给 Cheers。
...
type = "stdio"
command = "opencode"
args = ["acp", "--cwd", "/Users/haowei/Projects/Cheers"]

[accounts."opencode-acp".policy.workspace]
default_cwd = "/Users/haowei/Projects/Cheers"
allowed_roots = ["/Users/haowei/Projects/Cheers"]
...
```bash
export CHEERS_BOT_TOKEN=agb_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
cce-acp-connector run --config ./cheers-daemon.toml
```

后台守护进程：

```bash
cce-acp-connector start --config ./cheers-daemon.toml --name opencode-acp
...
```text
@opencode-acp 请读取当前消息并回复一句你已经通过 ACP 接入 Cheers
```

测试图片输入：

1. 在 Cheers 消息框上传一张图片。
2. 发送：
...
1. ACP agent 在 `agent_message_chunk` 中返回 `resource` 内容，connector 会上传到 Cheers 并把 `file_id` 附到最终回复。
2. ACP agent 在最终文本里提到 `file://...` 或 Markdown 文件链接，且文件位于 `agent.cwd` 内，connector 会扫描并上传新增或修改过的文件。

可以在 Cheers 中这样测试：

```text
@opencode-acp 在当前工作区生成一个 outputs/acp-test.md，内容写一段 Cheers ACP 文件回传测试，然后把文件链接返回给我
```
...
```bash
openclaw channels status --probe
openclaw channels logs | grep cheers | tail -100
```

常见原因是 Bot 未加入频道，或 OpenClaw 配置中的 account 没有写到 `channels.cheers.accounts` 下。
...
在 Cheers 管理界面对 Agent Bridge Bot 执行 rotate token，拿到新的 `agb_...` 后：

- OpenClaw（废弃/遗留）：更新 `~/.openclaw/openclaw.json`，再 `openclaw daemon restart`；有维护窗口时建议迁移到 `/acp-bridge` 的 ACP Connector。
- ACP：更新 `cheers-daemon.toml`，再 `cce-acp-connector restart --name <name>`。
