...
本文说明如何在 AgentNexus 中配置 kkFileView，用于增强 Office、WPS、OFD、压缩包、CAD、EPUB 等复杂文档的在线预览能力。

## 适用范围

AgentNexus 采用“内置预览 + kkFileView 增强复杂文档”的混合方案：

| 文件类型 | 预览方式 | 说明 |
| --- | --- | --- |
| 图片、PDF、HTML、Markdown、纯文本 | AgentNexus 内置预览 | 由前端直接展示，HTML 以受限 iframe 预览 |
...
```text
AgentNexus 站点: https://agentnexus.example.com
kkFileView 地址: https://agentnexus.example.com/preview
kkFileView 入口: https://agentnexus.example.com/preview/onlinePreview
```

不要把 `KKFILEVIEW_BASE_URL` 配成裸域名 `https://agentnexus.example.com`，否则后端会生成 `/onlinePreview`，与当前 Nginx 的 `/preview/` 代理路径不一致。
...
4. 后端生成一个短期签名下载地址：
   ...
   ```text
   https://agentnexus.example.com/api/v1/files/{file_id}/public-preview?token=...&fullfilename=...
   ```

5. 后端把该地址 Base64 编码后拼到 kkFileView viewer URL：

   ```text
   https://agentnexus.example.com/preview/onlinePreview?url=...
   ```
...
kkfileview:
  ...
  environment:
    ...
    - KK_BASE_URL=${KKFILEVIEW_BASE_URL:-https://agentnexus.example.com/preview}
    - KK_TRUST_HOST=${KKFILEVIEW_TRUST_HOST:-agentnexus.example.com}
...
`${KKFILEVIEW_HOST_BIND:-127.0.0.1}:${KKFILEVIEW_HOST_PORT:-8012}:8012` 表示 kkFileView 默认只暴露给服务器本机，不直接暴露到公网。公网访问统一走 `https://agentnexus.example.com/preview/`。
...
项目内置 `frontend` 容器通过 Docker 内网访问 `kkfileview:8012`，所以使用内置前端代理时，修改 `KKFILEVIEW_HOST_PORT` 不影响浏览器访问的 `https://agentnexus.example.com/preview/`。
...
```bash
PUBLIC_BASE_URL=https://agentnexus.example.com
KKFILEVIEW_ENABLED=true
KKFILEVIEW_BASE_URL=https://agentnexus.example.com/preview
KKFILEVIEW_TOKEN_TTL_SECONDS=600

KKFILEVIEW_HOST_BIND=127.0.0.1
KKFILEVIEW_HOST_PORT=8012
KKFILEVIEW_IMAGE=keking/kkfileview:latest
KKFILEVIEW_TRUST_HOST=agentnexus.example.com
...
| 变量 | 默认值 | 作用 |
| --- | --- | --- |
| `PUBLIC_BASE_URL` | `https://agentnexus.example.com` | kkFileView 拉取源文件时使用的 AgentNexus 公网地址 |
| `KKFILEVIEW_ENABLED` | `true` | 是否启用 kkFileView 复杂文档预览 |
| `KKFILEVIEW_BASE_URL` | `https://agentnexus.example.com/preview` | 后端返回给前端 iframe 的 kkFileView 地址 |
...
| `KKFILEVIEW_TRUST_HOST` | `agentnexus.example.com` | kkFileView 允许访问的主机名 |
...
如果公网 Nginx 只把请求转发到 AgentNexus `frontend` 容器，则项目内置的 `frontend/nginx.conf` 已经包含：
...
这种方式下，外层 Nginx 只需要把 `https://agentnexus.example.com` 转发给前端容器即可。
...
server {
    ...
    server_name agentnexus.example.com;
...
3. 检查公网路径：
   ...
   ```bash
   curl -I https://agentnexus.example.com/preview/
   ```

4. 登录 AgentNexus，上传 `docx`、`xlsx` 或 `pptx` 文件，点击预览。
...
```text
https://agentnexus.example.com/api/v1/files/{file_id}/public-preview?token=...
```
...
```yaml
extra_hosts:
  - "agentnexus.example.com:host-gateway"
```
...
7. 不要把 `KKFILEVIEW_TRUST_HOST` 配成 `*`，优先固定为 `agentnexus.example.com`。
...
wget -S -O /dev/null https://agentnexus.example.com/preview/
```
...
刷新 AgentNexus 页面后重新点击预览即可重新生成 token。生产环境应固定 `JWT_SECRET_KEY`。
...
使用项目内置前端代理时，外部访问地址仍然是 `https://agentnexus.example.com/preview/`，不要把 `KKFILEVIEW_BASE_URL` 改成带 `:18012` 的地址。
...
- `kkfileview` 容器运行正常
- `https://agentnexus.example.com/preview/` 可访问
- `kkfileview` 容器能访问 `https://agentnexus.example.com/api/v1/...`
