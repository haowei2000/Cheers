...
Docker 默认从 `.env.example` 复制时，后端宿主机端口是 `8002`；本地 `uvicorn` 开发常用 `8000`。下面示例用变量表示：

```bash
export AGENTNEXUS_BASE_URL=http://localhost:8002
export AGENTNEXUS_WS_BASE=ws://localhost:8002
```
...
curl -X POST "$AGENTNEXUS_BASE_URL/docs/agent-bridge/register" \
  ...
  -d '{
    ...
    "account_password": "admin#Nexus2024",
    "agent_id": "main",
    "scope": "private"
  }'
```
