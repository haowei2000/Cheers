...
4. 填写 **Name / 名称**、**Description / 描述**、**System prompt / 系统提示词**、**User template / 用户模板**。
...
普通用户能看到内置模板、系统/管理员创建模板、自己创建的模板。管理员能看到全部模板。
...
删除非内置模板时，引用该模板的 Bot 会被解绑，即 `BotAccount.template_id` 会被置空。当前模板服务不会同步清理频道成员上的 `ChannelMembership.template_id`，因此如果仍有频道 Bot 覆盖引用该模板，删除可能因外键约束失败。删除可能被频道覆盖使用的模板前，应先清理对应频道 Bot 覆盖。
