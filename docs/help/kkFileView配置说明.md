...
```text
AgentNexus 站点: https://agentnexus.epichust.com
kkFileView 地址: https://agentnexus.epichust.com/preview
kkFileView 入口: https://agentnexus.epichust.com/preview/onlinePreview
```

不要把 `KKFILEVIEW_BASE_URL` 配成裸域名 `https://agentnexus.epichust.com`，否则后端会生成 `/onlinePreview`，与当前 Nginx 的 `/preview/` 代理路径不一致。
...
4. 后端生成一个短期签名下载地址：
   ...
   ```text
   https://agentnexus.epichust.com/api/v1/files/{file_id}/public-preview?token=...&fullfilename=...
   ```

5. 后端把该地址 Base64 编码后拼到 kkFileView viewer URL：

   ```text
   https://agentnexus.epichust.com/preview/onlinePreview?url=...
   ```
...
kkfileview:
  ...
  environment:
    ...
    - KK_TRUST_HOST=${KKFILEVIEW_TRUST_HOST:-agentnexus.epichust.com}
...
```bash
PUBLIC_BASE_URL=https://agentnexus.epichust.com
...
| 变量 | 默认值 | 作用 |
| --- | --- | --- |
| `PUBLIC_BASE_URL` | `https://agentnexus.epichust.com` | kkFileView 拉取源文件时使用的 AgentNexus 公网地址 |
...
这种方式下，外层 Nginx 只需要把 `https://agentnexus.epichust.com` 转发给前端容器即可。
...
server {
    ...
    server_name agentnexus.epichust.com;
...
3. 检查公网路径：
   ...
   ```bash
   curl -I https://agentnexus.epichust.com/preview/
   ```
...
```text
https://agentnexus.epichust.com/api/v1/files/{file_id}/public-preview?token=...
```
...
```yaml
extra_hosts:
  - "agentnexus.epichust.com:host-gateway"
```
...
7. 不要把 `KKFILEVIEW_TRUST_HOST` 配成 `*`，优先固定为 `agentnexus.epichust.com`。
...
wget -S -O /dev/null https://agentnexus.epichust.com/preview/
```
...
- `kkfileview` 容器运行正常
- `https://agentnexus.epichust.com/preview/` 可访问
- `kkfileview` 容器能访问 `https://agentnexus.epichust.com/api/v1/...`
