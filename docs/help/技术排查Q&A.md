...
| **启动报错 `no such column: bot_accounts.description`** | 旧版 `bot_accounts` 表未升级到新结构 | 1）执行迁移；2）检查 `bot_accounts` 是否包含 `description/model_id/template_id/custom_system_prompt` | 补齐缺失列并确保内置 bot 的 model/template 有效 |
