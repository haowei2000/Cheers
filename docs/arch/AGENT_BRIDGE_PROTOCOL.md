...
| `4409` | `IDLE_TIMEOUT` | 否/可重连 | 90s 无入站帧（心跳丢失），服务端回收连接 |
...
| server probe interval | 30s | Backend 每 30s 发 WS ping（对端 WS 栈自动回 pong） |
| heartbeat idle close | 90s | Backend 关闭 90s 无入站帧的连接（close code `4409`） |
...
- `trace` handler。
- heartbeat idle close。（已实现：服务端 30s WS ping 探活 + 90s 无入站帧关闭 `4409`，
  连接建立/断开含原因记入 `bot_connection_events`。）
