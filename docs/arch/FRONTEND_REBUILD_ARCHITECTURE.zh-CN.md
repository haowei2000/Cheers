...
AgentNexus 是登录后的实时工作台，不是公开营销站点。核心工作流是：
...
AgentNexus 已经有 Rust Backend 负责 API、WebSocket、auth、permissions、files、sessions 和
...
重做不应从引入大型视觉组件库开始。AgentNexus 需要的是密集、可扫描、可重复操作的工作台，
...
AgentNexus 应该像一个实时 agent operations workbench。主内容保持安静可读；所有正在变化的
状态都要明显“活着”。
