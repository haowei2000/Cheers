...
> ⚠️ 历史设计，已废弃 — `channel.memory` 动词指向已 DROP 的 `memory_entries` 表（`0003_decentralized_mesh.sql:89`），ANCHOR/DECISIONS/PROGRESS 分层模型不再存在；改用 `fs.*` 读取 `memory_files` 文件树（见 CURRENT MODEL）。
