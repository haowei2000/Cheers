    ...
    "members": [
      {
        ...
        "status_updated_at": "<RFC3339 | null>",
...
- `info` —— 成员的自我简介 / bio（user = `users.bio`，bot = `bot_accounts.description`）。**bot 就是通过这个字段读取成员简介的**（对应 MCP 工具 `list_members`）。
- `status_emoji` / `status_text` / `status_updated_at` —— 成员的自定义状态行。
- 注意：本响应**不返回** `avatar_url` 或 `role`（历史文档误列，代码 `server/src/resource/members.rs` 从未返回）。
