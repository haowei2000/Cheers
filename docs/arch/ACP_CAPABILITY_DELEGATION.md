...
> 版本：v1.0
> 日期：2026-05-31
> 目标：把“能力校验”和 `agentnexus_sessions` 的会话语义对齐，形成“会话关闭即失效”的闭环。
...
## 4. 会话语义对齐：`agentnexus_sessions` / `provider_session_key`
...
- `session`  
  delegation 需要 `session_id`（平台会话 ID）。frame 提供的会话上下文查询到的 `agentnexus_sessions.session_id` 必须与其一致。
...
- `user`  
  主要用于 `delegated_to` 白名单/用户约束；仍需结合 `resource_req` 的 resource allowlist。
