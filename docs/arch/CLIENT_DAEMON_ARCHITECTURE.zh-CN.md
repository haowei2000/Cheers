...
| Local Loopback Resource Server | MCP tool call 的本机入口 | cheers-mcp-server、BridgeRuntime | 不在本地校验平台 Grant。 |
| cheers-mcp-server | 暴露 Cheers 资源类 MCP tools | loopback endpoint | 不打开自己的 Agent Bridge 连接。 |
...
daemon 不直接管理 `cheers-mcp-server` 子进程。MCP 生命周期属于 ACP `mcpServers` 机制。
...
Provider permission request 必须通过 Cheers 解决，不能在本地伪造。
...
- 不让 daemon 直接 spawn 或持有 `cheers-mcp-server`。
- 不在本地判断 Cheers channel/file/grant 权限。
