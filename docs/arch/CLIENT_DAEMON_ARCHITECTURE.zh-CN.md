...
| `bridge_runtime.rs` | 计划中的编排层：task/cancel/session/config/permission/resource 路由。 |
| `runtime_adapter.rs` | 计划中的 adapter trait 和 runtime event。 |
| `acp_adapter.rs` | 计划中的 ACP stdio JSON-RPC adapter。 |
| `loopback.rs` | 计划中的 MCP tool call 本机 endpoint。 |
| `state.rs` | 计划中的本地 provider session 映射存储。 |
...
1. 稳住 Rust supervisor：metadata、logs、start/stop/status、进程校验。
2. Rust Agent Bridge frame 类型对齐 `AGENT_BRIDGE_PROTOCOL.md`。
3. 实现 BridgeSession：auth、hello、ready、membership state、frame routing。
4. 实现 BridgeRuntime，并保留真实 adapter 边界。
5. 实现 AcpAdapter：ACP initialize/session/new/load/prompt/cancel 和 session/update 事件。
6. 实现 State Store：provider session mapping。
7. 实现 Local Loopback Resource Server 和 MCP 注入。
8. 实现 permission request/resolution 流。
9. 实现 config update/config option 流。
10. 最后再把默认 `run` 从 TypeScript foreground runtime 切到 Rust runtime。
