...
> **状态：** 🛠 规划 —— 持续维护的工程改造计划，**非**当前状态的权威参考。当前架构以 [ARCHITECTURE_OVERVIEW.md](./ARCHITECTURE_OVERVIEW.md) 和实际代码为准。
...
└──────┬──────────────────────────┬───────────────────────────────────┘
       ...
       ▼                          ▼          （botToken 鉴权）
  PG（状态真相）          ┌───────┴────────────────────────┐
  Redis（路由/广播，易失） │ packages/cheers-mcp-server  │  stdio MCP ↔ resource_req 桥
  S3/rustfs（文件）       │ packages/cheers-acp-        │  本地 daemon/connector，
...
- **cheers-mcp-server**（stdio）：把 MCP `tools/call` 一对一翻译成 `resource_req`（`build_resource_call`），让 Claude/Codex/Cursor 等 MCP 宿主无感接入。无独立状态。
- **cheers-acp-connector-rs**：本地 daemon。`bridge.rs` 管双 WS 连接/重连退避/能力协商，`bridge_runtime.rs`（2467 行，改造对象 R11）跑 task→ACP session 的运行时循环，`loopback.rs` 本地回环默认拒绝（commit 9312fdc）。
...
```jsonc
{
  ...
  "provider_session_key": "cheers:workspace:{ws}:bot:{bot}",
...
```
广播侧（fanout）：
  stream.rs ─ broadcast_channel ─▶ serde 序列化 ─▶ PUBLISH cheers:rt:channel:{id}
     ─▶ Redis ─▶ 本进程 psubscribe("cheers:rt:channel:*") 收到
     ─▶ serde 反序列化 ─▶ InProcessFanout 本地查订阅 ─▶ try_send 入连接队列

bot 路由侧（registry）：
  dispatcher ─▶ PUBLISH cheers:bot:{id}:control ─▶ Redis
     ─▶ 持有该 bot control WS 的实例 SUBSCRIBE 收到 ─▶ forward_loop ─▶ WS 下发
  在线判定：SET cheers:bot:{id}:online EX 30（心跳续期）+ PUBLISH 订阅者数
```

两个结构性问题（改造项 R1/R7 的依据）：

1. **单实例下纯开销**：每帧多付「序列化→Redis 往返→反序列化」，最终还是回到本进程的 InProcessFanout。
2. **firehose 订阅不可扩展**：`psubscribe cheers:rt:channel:*` 让**每个实例接收全平台所有频道的流量**再本地过滤，多实例时每实例负载 = 全网流量，水平扩容失义。
...
- **方案 B——走完多实例**（如果近期确要 ≥2 实例）：
  1. `StreamRegistry` 迁 Redis：`HSET cheers:stream:{msg_id}`（entry 字段）+ `INCR …:seq`，TTL 兜底孤儿；
...
- **现状**：`psubscribe cheers:rt:channel:*` 全量火喉（`redis_fanout.rs:125-126`）。
...
| 键/主题 | 用途 | 生命周期 |
|---|---|---|
| `cheers:rt:channel:{channel_id}` | 频道事件 pub/sub | 易失 |
| `cheers:rt:user:{user_id}` | 用户级通知 pub/sub | 易失 |
| `cheers:bot:{bot_id}:control` | task 帧路由 pub/sub | 易失 |
| `cheers:bot:{bot_id}:data` | data 帧路由 pub/sub | 易失 |
| `cheers:bot:{bot_id}:online` | 在线标记 | `SET … EX 30`，心跳续期 |
