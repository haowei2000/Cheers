...
- channel_id **不靠 Bridge 反查 session**，由 worker dispatch 时带下来（仅作缓存提示）；**PG `Message.channel_id` 是裁决**（见 §8.3 规则 R1）。
- ACP 的 `delta` 帧虽自带 `seq`，但**不可直接透传给客户端**——client-facing seq 由 Bridge 盖戳（见 §8.3 规则 R2）。
...
### 8.3 回流的安全与正确性硬规则（实现必须遵守）

外部 ACP agent 的信任级别是 `untrusted` / `trusted_partner`（见 [BOT_CONFIG_LAYERING](./BOT_CONFIG_LAYERING.md) 的 `trust_level`）。
connector 上报的任何字段（`msg_id` / `seq` / `channel_id`）都**不可盲信**。以下为强制规则：

| 规则 | 内容 | 防的问题 |
|------|------|---------|
| **R1 所有权校验** | 收到 `delta/done/trace{msg_id}` 时，必须校验该 msg_id 的占位 **owner == 当前 bot**（`Message.sender_id == bot_id`），且占位仍 active（`is_partial` 或空内容）。校验以 **PG 为准**，dispatch 缓存的 `channel_id` 仅作命中加速。 | **跨频道注入**：被攻破/有 bug 的 connector 用别人的 msg_id 把内容打进别的频道 |
| **R2 seq 服务端盖戳** | client-facing `seq` 由 **Bridge（持 connector 的 owner 实例）盖戳**，**不透传** connector 自报的 seq。盖戳后再 publish 到 `rt.stream.{channel_id}.{msg_id}`。 | 外部 agent 乱报 seq → 击穿 [WIRE §5](./WIRE_PROTOCOL.md) 的客户端去重/排序 |
| **R3 确定性占位 id** | 占位 id 由 `(trigger_msg_id, bot_id)` **确定性派生**；worker 重投/重跑时 **upsert 同一占位**，而非新建。 | **重复气泡**：claim 重投（[TASK_DELIVERY §4](./TASK_DELIVERY.md)）导致重跑建出第二个占位 |
| **R4 finalize 守卫** | 占位 finalize 后（`is_partial=false` 且有内容）拒绝迟到 delta。finalize 状态**从 PG 可推导**，故跨实例/重连后仍生效。 | 网络重排/重放产生的 finalize 后迟到 delta |

> R1/R2 的本质：**Bridge 是该 bot 回流的权威**——所有权和 seq 都由 Bridge 按服务端真相裁决，connector 只是内容来源。
> 这与 [TASK_DELIVERY §4](./TASK_DELIVERY.md)「claim owner 拥有 seq」一脉相承：对 ACP，owner = 持 connector 的 Bridge 实例。

### 8.4 delta / send 的有意非对称（保留，勿强求统一）

| 输出模式 | 关联键 | 语义 |
|---------|--------|------|
| `delta` / `done` | **msg_id**（流进预建占位） | 续写某个气泡；channel 由 R1 从占位裁决 |
| `send`（agent 主动发新消息） | **channel_id**（帧直接带，建全新 Message） | 新建消息；R1 退化为「校验 bot 是该 channel 成员」（现有 `check_bot_in_channel`） |

两种模式用不同关联键是**有意的**：delta 必须打进特定气泡，send 是新建。**保留此非对称**，不强求统一。
注意 send 路径同样要做成员校验（现有 `_handle_data_send` 已校验 `check_bot_in_channel`），与 R1 等价的信任边界。
...
| dispatch 带 channel_id | worker→Bridge 的 dispatch payload 带 `{channel_id, msg_id}`，仅作缓存提示 |
| cancel 跨进程 | REST 取消经 `acp.cancel.{bot_id}` 路由到 owner 实例触发 StreamState cancel |
| R1 所有权校验 | delta/done/trace 以 PG `Message.sender_id==bot_id` 裁决，不盲信 dispatch 缓存 |
| R2 seq 服务端盖戳 | Bridge 盖 client-facing seq，不透传 connector 自报 seq |
| R3 确定性占位 id | 占位 id 由 `(trigger_msg_id, bot_id)` 派生，重跑 upsert（同 [TASK_DELIVERY](./TASK_DELIVERY.md) #5） |
