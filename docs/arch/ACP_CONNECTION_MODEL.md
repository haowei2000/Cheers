...
### Phase 0（Python 侧，不涉及 Rust）

| 改动 | 说明 |
|------|------|
| 内置 bot 改走 Agent Bridge | `HttpBotAdapter` 等不再进程内直接调用，改为通过 Agent Bridge WS 协议 |
| resource_req/res handler | 在 Python 的 agent_bridge routes 中实现 resource 处理（原型） |
| Agent Service 独立进程 | 内置 bot 逻辑拆到独立 Python 进程，通过 WS 连接 |

### Phase 1（Rust Backend PoC）
