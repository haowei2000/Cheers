...
## 9. 对照 Zed：本地 UI 客户端 vs 中继客户端

同一套 ACP 协议、同一个 agent（`claude-agent-acp`），Zed 与 Cheers 的复杂度差异**全部来自拓扑**——「人」离 agent 多远。

```
Zed（本地 UI 客户端，单人同步）
  编辑器 ── 子进程 claude-agent-acp
    request_permission → Zed 在 agent 面板内联渲染按钮 → 你点 → 立即回 outcome
  「always」→ 响应带 updatedPermissions:[{addRules}] → SDK 写本地规则（acp-agent.js:1118）

Cheers（中继客户端，远程多人异步）
  agent ─ACP→ connector(headless,无 UI) ─bridge→ 平台 ─广播→ 频道(可能多人)
    ↑ 有权限的人点 ─resolve→ 平台 ─控制帧→ connector ─outcome→ agent
```

| 维度 | Zed | Cheers |
|---|---|---|
| client 形态 | 本地编辑器,**有 UI** | connector 是 **headless 中继**,本地无 UI |
| 人在哪 | 就在 agent 旁,1 人 | **远程**,频道里**可能多人** |
| 渲染 | 内联按钮,**同步** | 转发→频道卡片,**异步** |
| 谁能批 | 不用问(就你) | 需 **who-can-resolve**(bot owner/委托)+ 审计 |
| 「always」 | SDK 写本地 allow 规则 | 交给 agent 侧,平台不存 |

> 结论：Zed 简单是因为它是「本地 UI 客户端」,`request_permission` 直接画成按钮。Cheers 的 connector 是「**中继客户端**」,被迫多出「转发 / 谁能批 / 裁决回灌 / 审计」——本质是把 Zed 那张本地卡片，拆成**跨网络、多用户、可审计**的版本。`request_permission` 是两者唯一共享的协议事件。

---

## 10. 工具集由 agent 决定（`AskUserQuestion` 等不在 ACP 路径）

**事实源在 agent 的 ACP 实现里，不在客户端。** 客户端（Zed / Cheers connector）只声明 `fs`/`terminal` 等少数能力（`config.rs:918` `client_capabilities()`），**无法**给 agent 增删工具。

- `AskUserQuestion`（多选澄清）是**交互式 Claude Code CLI/TUI** 的工具；headless 的 **Agent SDK 路径不暴露**它。验证：`claude-agent-acp/dist/acp-agent.js` **零** `AskUserQuestion` 引用,只暴露 `requestPermission` / `ExitPlanMode` / `setPermissionMode` / `canUseTool`。
- 因此模型在 ACP 下会说「AskUserQuestion isn't available here」并降级为**文本提问**——这跟客户端是 Zed 还是 Cheers **无关**(Zed 接 Claude 也一样没有)。
- 易混淆点：Zed 里看到的「计划评审」UI 是 **`ExitPlanMode`**(映射到 `request_permission`),**不是** `AskUserQuestion`。`ExitPlanMode` 两边都有——它**无条件**触发 `request_permission`(`acp-agent.js:1064`),是验证审批闭环最稳的触发器。

> 推论：要不要支持 `AskUserQuestion` 式的「问人」,取决于 **agent 端**是否把它作为工具暴露并映射到某个 ACP 原语;换个 agent(如 Gemini ACP)可能有不同的问答原语。客户端补不了。
