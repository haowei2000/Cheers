    ...
    "agentnexus": {
      ...
      "accounts": {
        "my-bot": {                    // 任意 ID，对应 AgentNexus 里的一个 Agent Bridge Bot
          ...
          "controlUrl": "ws://your-host:8000/ws/agent-bridge/control",  // 必填
          "dataUrl":    "ws://your-host:8000/ws/agent-bridge/data",     // 必填
          "advanced": {                              // 可选，全部有合理默认
...
```bash
curl -H "X-Agent-Bridge-Token: <BRIDGE_TOKEN>" http://localhost:8000/api/v1/agent-bridge/status
...
| 连不上 / `ECONNREFUSED` | URL 写错（端口、host、是否走反代） | 默认端口见 `.env` 的 `BACKEND_HOST_PORT`，公开模板默认为 `8000` |
