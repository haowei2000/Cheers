#!/usr/bin/env bash
# Build the Connector and stage it as a Tauri sidecar. Cheers tools are served
# by the Gateway's native HTTP MCP endpoint, so no local MCP binary is bundled.
