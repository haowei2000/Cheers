...
from app.contracts.messages import MessageDTO, MessageUpdateDTO


def _wire_payload(value: Any) -> dict:
...
class MessageCreated(Event):
    ...
    def to_ws_frame(self) -> dict:
        return {"type": "message", "data": _wire_payload(self.data)}

    def to_sse(self) -> tuple[str, dict]:
        return "message", _wire_payload(self.data)
...
class BotMessagePlaceholder(Event):
    ...
    def to_sse(self) -> tuple[str, dict]:
        return "bot_message", _wire_payload(self.data)
...
class MessageDone(Event):
    ...
    update: MessageUpdateDTO | None = None

    def _payload(self) -> dict:
        if self.update is not None:
            return self.update.to_wire()
