...
const LAYERS = ["ANCHOR", "PROGRESS", "DECISIONS", "FILES_INDEX", "RECENT", "MEMBERS", "TODO"] as const;

const LAYER_META: Record<string, {
  label: string; desc: string; icon: string;
  color: string; bgLight: string; borderColor: string;
  entryBased?: boolean; readonly?: boolean;
}> = {
  ANCHOR:      { label: "项目锚点", desc: "核心目标、约束、背景", icon: "⚓", color: "text-blue-600", bgLight: "bg-blue-50", borderColor: "border-blue-200", entryBased: true },
  PROGRESS:    { label: "项目进度", desc: "当前进度、已完成、下一步", icon: "📈", color: "text-teal-600", bgLight: "bg-teal-50", borderColor: "border-teal-200", entryBased: true },
  DECISIONS:   { label: "决策记录", desc: "重要决策及原因", icon: "📋", color: "text-purple-600", bgLight: "bg-purple-50", borderColor: "border-purple-200", entryBased: true },
  FILES_INDEX: { label: "资料索引", desc: "上传的文件与参考资料", icon: "🗂️", color: "text-amber-600", bgLight: "bg-amber-50", borderColor: "border-amber-200", readonly: true },
  RECENT:      { label: "近期动态", desc: "历史对话摘要", icon: "🕐", color: "text-green-600", bgLight: "bg-green-50", borderColor: "border-green-200", readonly: true },
  MEMBERS:     { label: "频道成员", desc: "用户与 Bot 能力一览", icon: "👥", color: "text-gray-600", bgLight: "bg-gray-50", borderColor: "border-gray-200", readonly: true },
  TODO:        { label: "待办事项", desc: "频道任务清单", icon: "✅", color: "text-rose-600", bgLight: "bg-rose-50", borderColor: "border-rose-200", readonly: true },
};

/* ── File card helper: parse FILES_INDEX markdown into cards ────────────── */

type FileCard = { filename: string; fileId: string; contentType: string; summary: string; time: string };
...
function parseFilesIndex(md: string): FileCard[] {
  ...
  return blocks.map((block) => {
    ...
    let fileId = "", contentType = "", summary = "", time = "";
    for (const line of lines.slice(1)) {
      const m = line.match(/^-\s*file_id:\s*`([^`]+)`/);
      if (m) { fileId = m[1]; continue; }
      const m2 = line.match(/^-\s*类型:\s*(.+)/);
      if (m2) { contentType = m2[1].trim(); continue; }
      const m3 = line.match(/^-\s*摘要:\s*(.+)/);
      if (m3) { summary = m3[1].trim(); continue; }
      const m4 = line.match(/^-\s*登记时间:\s*(.+)/);
      if (m4) { time = m4[1].trim(); continue; }
    }
...
type TimelineItem = { pageId: string; from: string; to: string; summary: string };

function parseRecentXml(xml: string): TimelineItem[] {
  if (!xml.trim()) return [];
  const items: TimelineItem[] = [];
  const re = /<page\s+id="([^"]*)"[^>]*from="([^"]*)"[^>]*to="([^"]*)">([\s\S]*?)<\/page>/g;
  let m;
...
function formatRange(from: string, to: string): string {
  try {
    ...
    const b = new Date(to);
    const df = a.toLocaleDateString("zh-CN", { month: "short", day: "numeric" });
    const tf = a.toLocaleTimeString("zh-CN", { hour: "2-digit", minute: "2-digit" });
    const tb = b.toLocaleTimeString("zh-CN", { hour: "2-digit", minute: "2-digit" });
    return `${df} ${tf} — ${tb}`;
  } catch {
    return `${from} — ${to}`;
  }
    ...
    <div className="border border-blue-200 rounded-lg overflow-hidden bg-white shadow-sm">
      ...
      <div className="min-h-[160px]">
        {previewMode ? (
          ...
          <textarea
            ...
            onKeyDown={(e) => { if (e.key === "Enter" && (e.metaKey || e.ctrlKey)) onSave(title, content); }}
          />
        )}
      </div>
      {/* Actions */}
      <div className="flex items-center justify-between px-3 py-2 bg-gray-50 border-t border-gray-100">
        ...
        <div className="flex gap-2">
          <button onClick={onCancel} className="text-xs px-3 py-1.5 rounded border border-gray-200 text-gray-500 hover:bg-gray-100">
            取消
          </button>
          <button
...
export default function MemoryPage({
  ...
  const loadEntries = useCallback((layer: string) => {
    setEntriesLoading(true);
    fetch(`${API}/channels/${channelId}/memory/?layer=${layer}`, { headers: authHeaders() })
      .then((r) => (r.ok ? r.json() : []))
      .then(setEntries)
      .catch(() => {})
  ...
  const switchLayer = (layer: string) => {
    ...
    if (layer === "TODO") { loadTodos(); if (!members.length) loadMembers(); }
  };
  ...
  const handleCreate = async (title: string, content: string) => {
    ...
    const res = await fetch(`${API}/channels/${channelId}/memory/`, {
      ...
      body: JSON.stringify({ layer: activeLayer, title: title || null, content }),
    }).catch(() => null);
    setSaving(false);
    if (res?.ok) { setAddingNew(false); loadEntries(activeLayer); }
  };

  const handleUpdate = async (entryId: string, title: string, content: string) => {
    setSaving(true);
    ...
    setSaving(false);
    if (res?.ok) { setEditingId(null); loadEntries(activeLayer); }
  };
  ...
  const handleTodoCreate = async () => {
    ...
    if (todoAssignee) { const [t, i] = todoAssignee.split(":"); assignee_id = i; assignee_type = t; }
  ...
  const renderEntryCards = () => {
    if (entriesLoading) return <div className="flex items-center justify-center py-20 text-gray-400">加载中…</div>;
      ...
      <div className="space-y-4">
        {entries.map((entry) =>
          editingId === entry.entry_id ? (
            ...
            >
              {/* Card header */}
              <div className={`flex items-center justify-between px-4 py-2.5 ${meta.bgLight} rounded-t-lg border-b ${meta.borderColor}`}>
                <div className="flex items-center gap-2 min-w-0">
                  {entry.title ? (
                    <h3 className="text-sm font-semibold text-gray-800 truncate">{entry.title}</h3>
                  ) : (
                    <h3 className="text-sm text-gray-400 italic truncate">无标题</h3>
                  )}
                </div>
                <div className="flex items-center gap-1.5 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                  <span className="text-[11px] text-gray-400 mr-1">{relativeTime(entry.updated_at)}</span>
                  <button
              ...
              {(entry.creator_type || entry.created_at) && (
                <div className="px-4 py-2 border-t border-gray-50 flex items-center gap-2 text-[11px] text-gray-400">
                  {entry.creator_type === "bot" && <span className="px-1.5 py-0.5 rounded bg-green-50 text-green-600">Bot</span>}
                  {entry.creator_type === "user" && <span className="px-1.5 py-0.5 rounded bg-blue-50 text-blue-600">用户</span>}
                  {entry.created_at && <span>{new Date(entry.created_at).toLocaleString("zh-CN")}</span>}
                </div>
              )}
            </div>
          ),
        ...
        {entries.length === 0 && !addingNew && (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-sm text-gray-400 mb-4">暂无记录，点击下方添加第一条</p>
            <button
    ...
    if (!cards.length) {
      return (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-sm text-gray-400">在频道中上传文件后，索引将自动生成</p>
        </div>
      );
    }
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {cards.map((f) => (
          <div key={f.fileId || f.filename} className="rounded-lg border border-amber-200 bg-white hover:shadow-md transition-shadow overflow-hidden">
            <div className="flex items-center gap-3 px-4 py-3 bg-amber-50 border-b border-amber-100">
              ...
              <div className="min-w-0 flex-1">
                ...
                {f.contentType && <p className="text-[11px] text-gray-400 truncate">{f.contentType}</p>}
              </div>
            </div>
            {f.summary && (
              <div className="px-4 py-2.5">
                <p className="text-xs text-gray-600 leading-relaxed line-clamp-3">{f.summary}</p>
              </div>
            )}
            <div className="px-4 py-2 border-t border-gray-50 flex items-center justify-between">
              ...
              <span className="text-[11px] text-gray-300 font-mono">{f.fileId.slice(0, 8)}</span>
            </div>
          </div>
        ))}
      </div>
            ...
            <div key={item.pageId || idx} className="relative">
              ...
              <div className="rounded-lg border border-green-200 bg-white hover:shadow-md transition-shadow overflow-hidden">
                <div className="px-4 py-2.5 bg-green-50 border-b border-green-100 flex items-center gap-2">
                  <span className="text-xs font-medium text-green-700">{formatRange(item.from, item.to)}</span>
                </div>
  ...
  const renderMembers = () => {
    if (membersLoading) return <div className="flex items-center justify-center py-20 text-gray-400">加载中…</div>;
    if (!members.length) {
      return (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <span className="text-5xl mb-4 opacity-20">👥</span>
    ...
    const sorted = [...members].sort((a, b) => (a.member_type === "bot" ? -1 : 1) - (b.member_type === "bot" ? -1 : 1));
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {sorted.map((m) => {
          ...
          return (
            <div key={m.member_id} className="flex items-center gap-3 rounded-lg border border-gray-200 bg-white px-4 py-3 hover:shadow-sm transition-shadow">
              <div className={`w-10 h-10 ${isBot ? "rounded-lg bg-[#2EB67D]" : "rounded-full bg-[#1264A3]"} flex items-center justify-center text-white text-sm font-bold flex-shrink-0`}>
                {initial}
              </div>
              <div className="min-w-0 flex-1">
              ...
              <span className={`text-[11px] px-2 py-0.5 rounded-full flex-shrink-0 font-medium ${isBot ? "bg-green-50 text-green-700" : "bg-blue-50 text-blue-700"}`}>
                {isBot ? "Bot" : "用户"}
              </span>
            </div>
          );
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-4 space-y-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleTodoCreate} className="px-4 py-1.5 text-sm bg-[#1264A3] text-white rounded-lg hover:bg-[#0f5a94] flex-shrink-0">
              添加
            </button>
          </div>
        </div>
        ...
        {todosLoading ? (
          ...
          <div className="space-y-4">
            ...
            {pending.length > 0 && (
              <div>
                <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">未完成 ({pending.length})</h3>
                <div className="space-y-2">
                  {pending.map((todo) => (
                    <div key={todo.todo_id} className="flex items-start gap-3 rounded-lg border border-gray-200 bg-white px-4 py-3 hover:shadow-sm transition-shadow group">
                      <input type="checkbox" checked={false} onChange={() => handleTodoToggle(todo)} className="mt-0.5 w-4 h-4 cursor-pointer flex-shrink-0" />
                      <div className="flex-1 min-w-0">
                        <p className="text-sm text-gray-800">{todo.content}</p>
                        {todo.assignee_id && (
                          <p className="text-xs text-gray-400 mt-0.5">→ {getMemberName(todo.assignee_id, todo.assignee_type!) ?? todo.assignee_id}</p>
                        )}
                      </div>
                      <button onClick={() => handleTodoDelete(todo.todo_id)} className="text-gray-300 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all flex-shrink-0" title="删除">
            ...
            {completed.length > 0 && (
              <div>
                <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">已完成 ({completed.length})</h3>
                <div className="space-y-2">
                  {completed.map((todo) => (
                    <div key={todo.todo_id} className="flex items-start gap-3 rounded-lg border border-gray-100 bg-gray-50/50 px-4 py-3 group">
                      <input type="checkbox" checked onChange={() => handleTodoToggle(todo)} className="mt-0.5 w-4 h-4 cursor-pointer flex-shrink-0" />
                      <p className="flex-1 text-sm text-gray-400 line-through">{todo.content}</p>
                      <button onClick={() => handleTodoDelete(todo.todo_id)} className="text-gray-300 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all flex-shrink-0">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                      </button>
                    </div>
                  ))}
  ...
  const renderContent = () => {
    switch (activeLayer) {
      case "ANCHOR": case "DECISIONS": case "PROGRESS": return renderEntryCards();
      case "FILES_INDEX": return renderFilesIndex();
      ...
      <div className="flex flex-1 bg-white rounded-lg m-3 shadow-2xl overflow-hidden">
        ...
        <nav className="w-56 flex-shrink-0 bg-gray-50 border-r border-gray-200 flex flex-col">
          ...
          <div className="px-4 py-4 border-b border-gray-200">
            ...
            {channelName && <p className="text-xs text-gray-400 mt-0.5">#{channelName}</p>}
          </div>
          {/* Layer list */}
            ...
            {LAYERS.map((layer) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className={`text-sm font-medium truncate ${active ? "text-[#1264A3]" : "text-gray-700"}`}>
                      {lm.label}
                    </p>
                    <p className="text-[11px] text-gray-400 truncate">{lm.desc}</p>
                  </div>
                </button>
              );
            })}
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 flex-shrink-0">
            ...
            <div className="flex items-center gap-2">
              ...
              {meta.entryBased && entries.length > 0 && (
                <span className="text-xs text-gray-400 px-2">{entries.length} 条记录</span>
              )}
              {meta.readonly && activeLayer !== "TODO" && (
                <span className="text-xs text-gray-400 px-2 py-1 rounded bg-gray-100">自动生成</span>
              )}
              <button
                onClick={onClose}
