...
export function AdminUsers() {
  ...
  async function act(u: UserSearchResult, suspend: boolean) {
    ...
    try {
      ...
      toast.success(
        suspend
          ? `Suspended ${u.display_name || u.username} (sessions revoked)`
          : `Unsuspended ${u.display_name || u.username}`
      );
    } catch (e) {
      ...
      <div className="bg-zinc-900 rounded-2xl border border-zinc-800 p-6 space-y-3">
        ...
        <div className="divide-y divide-zinc-800/60">
          ...
          {query.trim().length >= 2 && results.length === 0 && (
            <p className="text-xs text-zinc-600 py-3 text-center">No matching users</p>
          )}
        </div>
      </div>
    </section>
