...
export function BotPostureSection({ botId }: { botId: string }) {
  ...
  if (!perms) return <p className="text-compact text-zinc-400 px-1 py-2">Loading…</p>;
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {posture.allowed_modes.length > 0 ? (
              <UiSelect
                ...
                controlSize="regular" className="rounded-sm bg-zinc-800 text-compact text-zinc-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:opacity-50"
              >
            ...
            <span className="ml-auto text-compact text-zinc-400">
              agent: <code className="text-zinc-400">{posture.agent_type}</code>
            </span>
          </div>
          <p className="text-compact text-zinc-400 mt-2 leading-relaxed">
      ...
      <div className="rounded-sm bg-zinc-950/40 p-3">
        ...
        {configOptions && configOptions.advertised.length > 0 ? (
            ...
            {configOptions.advertised.map((opt) => {
              ...
              return (
                <div key={opt.id} className="flex items-center gap-2 flex-wrap">
                  <span className="text-compact text-zinc-400 min-w-[90px]">{opt.name}</span>
                  <UiSelect
                    value={current}
                    disabled={busy}
                  ...
                  {configOptions.desired[opt.id] != null && (
                    <span className="text-minimal text-indigo-400">override</span>
                  )}
                  {opt.category && (
                    <code className="ml-auto text-minimal text-zinc-400">{opt.category}</code>
                  )}
                </div>
              );
            })}
            <p className="text-compact text-zinc-400 mt-1 leading-relaxed">
          ...
          <div className="mt-2 space-y-2">
              ...
              <div className="space-y-1">
                ...
                {Object.entries(configOptions.desired).map(([id, value]) => (
                  <div key={id} className="flex items-center gap-2 text-compact">
                    ...
                    <span className="ml-auto text-minimal text-indigo-400">override</span>
                  </div>
                ))}
              </div>
            )}
            ...
            <p className="text-compact text-zinc-400 leading-relaxed">
