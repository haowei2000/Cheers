...
export function CreateBotDialog({
  ...
  return (
    <Dialog
      title={<span className="flex items-center gap-2"><Bot className="h-5 w-5 text-indigo-400" />Create bot</span>}
      onClose={onClose}
      maxWidth="max-w-lg"
    >
      <div className="space-y-4">
        ...
        {error && <p className="text-compact text-red-400 break-words">{error}</p>}
        <div>
          <label className="mb-1 block text-compact font-medium uppercase tracking-wide text-zinc-400">Username</label>
          <Input value={username} onChange={(event) => setUsername(event.target.value)} placeholder="research-assistant" controlSize="regular" />
        </div>
        <div>
          <label className="mb-1 block text-compact font-medium uppercase tracking-wide text-zinc-400">Display name</label>
