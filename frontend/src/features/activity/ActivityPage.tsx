    ...
    <main className="flex-1 overflow-y-auto">
      <div className="mx-auto w-full max-w-3xl space-y-7 px-4 py-6">
        {loading ? <SurfaceSpinner /> : approvals.length === 0 && invites.length === 0 && recent.length === 0 ?
          ...
          {approvals.length > 0 && <ItemSection label={<span className="flex items-center gap-2"><Shield className="h-3.5 w-3.5 text-amber-400" />Needs approval</span>} presentationLevel="medium" controlSize="regular">
            {approvals.map((approval) => <div role="listitem" key={approval.message_id} className="space-y-1 py-1">
              <p className="px-2 text-minimal uppercase tracking-wide text-zinc-400">{approval.channel_name ? `#${approval.channel_name}` : "Direct message"}</p>
          ...
          {recent.length > 0 && <ItemSection label={<span className="flex items-center gap-2"><History className="h-3.5 w-3.5" />Recent</span>} presentationLevel="medium" controlSize="regular">
            {recent.map((event) => <OperationsItem key={`${event.source}:${event.id}`} title={event.event_type.replaceAll("_", " ")} subtitle={new Date(event.created_at).toLocaleString()} status={<span className="text-compact text-zinc-400">Resolved</span>} />)}
          </ItemSection>}
        </>}
      </div>
    </main>
