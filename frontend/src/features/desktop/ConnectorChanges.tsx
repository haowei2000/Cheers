...
function DiffView({ diff }: { diff: string }) {
  if (!diff.trim()) {
    return <p className="text-compact text-content-muted px-2 py-1">No textual diff (binary or unchanged).</p>;
  }
  return (
    <pre className="text-compact bg-zinc-950 rounded-sm p-2 overflow-auto max-h-72 leading-reading">
      {diff.split("\n").map((line, i) => {
        const c = line[0];
        const cls =
...
export function ConnectorChanges({ name, openers }: { name: string; openers: Opener[] }) {
  ...
  if (error) {
    return <p className="text-compact text-removed-400">{error}</p>;
  }
  if (git === null) {
    return <p className="text-compact text-content-muted">Loading…</p>;
  }
  if (!git.is_repo) {
    return (
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 mb-3 text-compact">
        ...
        <span className="font-medium text-content-secondary truncate">{git.branch || "(detached)"}</span>
        {git.dirty ? (
          <PresenceDot contentSize="small" className="bg-amber-400" title="uncommitted changes" />
        ) : (
          <span className="text-compact text-content-muted">clean</span>
        )}
        {(git.ahead > 0 || git.behind > 0) && (
          <span className="text-compact text-content-muted tabular-nums">
      ...
      {git.files.length === 0 ? (
        <p className="text-compact text-content-muted">No changed files.</p>
      ) : (
            ...
            <ItemGroup key={f.path} className="text-compact">
              <WorkbenchItem containerRole="presentation" title={f.path}
                ...
                {primaryOpener && (
                  <UiButton action="open" variant="plain"
                    ...
                    className="text-content-primary hover:text-content-strong shrink-0"
                    onClick={() =>
                ...
                <UiButton action="discard" variant="plain"
                  ...
                  className="text-removed-400 hover:text-removed-300 disabled:opacity-50 shrink-0"
              ...
              {open === f.path && (
                <div className="ml-8 mt-1 mb-1">
                  {diffLoading ? (
                    <p className="text-compact text-content-muted px-2">Loading diff…</p>
                  ) : (
                    <DiffView diff={diff} />
                  )}
                </div>
