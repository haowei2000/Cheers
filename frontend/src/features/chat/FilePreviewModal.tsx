  ...
  return (
    <div
      className={`flex items-center justify-center gap-2 py-12 text-regular ${
        tone === "error" ? "text-danger-400" : "text-content-muted"
      }`}
    >
      {children}
    </div>
...
function TextBody({ file }: { file: FileInfo }) {
  ...
  return (
    <pre className="max-h-[70vh] overflow-auto rounded-sm bg-zinc-900 p-4 text-regular leading-reading">
...
export function FilePreviewModal({ file, onClose }: { file: FileInfo; onClose: () => void }) {
  ...
  return (
    <Dialog title={title} onClose={onClose} maxWidth="max-w-4xl">
      ...
      <div className="flex items-center justify-between border-t border-zinc-800 pt-2 text-compact text-content-muted">
        ...
        <UiButton action="download" content="iconText" variant="plain"
          ...
          controlSize="regular" className="inline-flex items-center gap-2 rounded-sm bg-zinc-800 text-content-primary hover:bg-zinc-700"
        >
