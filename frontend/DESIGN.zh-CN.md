...
| 卡片、弹窗、popover | `bg-zinc-900`，边框 `border-zinc-800` |
...
- Focus：`focus:ring-2 focus:ring-indigo-500`（输入框加 `focus:border-transparent`；按钮用 `focus-visible:`）——**禁止**用 `focus:border-indigo-*` 替代
...
单行文本用 `<Input>`。select / textarea 在共享组件出现前镜像它：

```tsx
// select / textarea 标准写法
...
| 表面 | 写法 |
|---|---|
| 模态弹窗（用 `<Dialog>`） | 遮罩 `bg-black/50`，卡片 `rounded-xl border border-zinc-800 bg-zinc-900 p-4` |
| 锚定 popover | `rounded-xl border border-zinc-800 bg-zinc-900 shadow-xl` |
| 自动补全 / 菜单列表 | 同 popover，紧凑列表可用 `rounded-lg` |
| 可拖拽窗口（用 `<FloatingPanel>`） | `rounded-xl border-zinc-700/80 bg-zinc-900/95 shadow-2xl ring-1 ring-black/40` |
